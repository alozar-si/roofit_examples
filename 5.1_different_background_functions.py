...
for i in range(hh.GetNbinsX()):
    hh.SetBinContent(i+1, hh.GetBinContent(i+1) + i + baseline+ random.exponential(noise) // 1)
_file0 = TFile("data_gaus_linear.root", "RECREATE")
hh.Write()
_file0.Close()"""
_file0 = TFile("data_gaus_constant.root", "READ")
hh = _file0.Get("hh")
...
sigma = RooRealVar("sigma","WidthofGaussian",3,-5,5)
gauss = RooGaussian("gauss","gauss(x,mean,sigma)",x,mean,sigma)

# Construct the background pdf
a0 = RooRealVar("a0", "a0", 0.001, 0, 5)
a1 = RooRealVar("a1", "a1", 0.5, 0, 100)
bkg_constant = RooPolynomial("bkg_constant", "bkg_constant", x) 
bkgPDF = RooPolynomial("px", "px", x, RooArgList(a0))

# Construct signal+bkg PDF
f = RooRealVar("f", "f", 0.6, 0, 1)
model = RooAddPdf("model", "model", RooArgList(gauss, bkg_constant), RooArgList(f))
...
model_n.plotOn(
    xframe,
    RooFit.Range("signal"),
    RooFit.Components("bkg_constant"),
...
c1.Print("5.1_different_background_functions.pdf")
