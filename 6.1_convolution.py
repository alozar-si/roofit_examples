...
# Create convolution PDF and fit. Extra: combine pdf with 1st order polynom
# # # # # # # # # # # # # # # # # #
...
# Construct BW
mbw = RooRealVar("mbw","mean bw",5.,-20,20)
sbw = RooRealVar("sbw","sigma bw",1,0.1,10)
...
frame = t.frame(RooFit.Title("landau (x) gauss convolution"))
data.plotOn(frame)
lxg.plotOn(frame)
#Plot components
# ------------
# ploting only component does not work, it plots same as the convoluted function
lxg.plotOn(frame, RooFit.Components("gauss")) 
lxg.plotOn(frame, RooFit.Components("landau"))
landau.plotOn(frame,RooFit.LineStyle(kDashed))
# Draw frame on canvas
c1 = TCanvas("6.1_convolution","6.1_convolution",600,600)
#gPad->SetLeftMargin(0.15)
frame.GetYaxis().SetTitleOffset(1.4) 
frame.Draw()
c1.Print("6.1_convolution.pdf")
...
bwxg.plotOn(frame2,RooFit.LineStyle(kDashed), RooFit.Components(
    "gauss"))
# Draw frame on canvas
c2 = TCanvas("6.1_convolution_bw_x_gauss","6.1_convolution_bw_x_gauss",600,600)
#gPad->SetLeftMargin(0.15)
frame2.GetYaxis().SetTitleOffset(1.4) 
frame2.Draw()
c2.Print("6.1_convolution_bw_x_gauss.pdf")
